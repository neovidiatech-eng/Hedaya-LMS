...
import { Eye, EyeOff, Check, User, Mail, Lock, Users, UserPlus } from "lucide-react";
...
export default function Register({ onRegisterSuccess }: RegisterProps) {
  ...
  // Group Plan UI state (Front-end preview)
              ...
              {filteredPlans?.map((pkg) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-2xl border-2 transition-all text-start relative overflow-hidden flex flex-col justify-between min-h-[95px] cursor-pointer ${
                  ...
                  >
                    {selectedPackage === pkg.id && (
                      <div
                        className={`absolute top-0 ${language === "ar" ? "left-0 rounded-br-xl" : "right-0 rounded-bl-xl"} w-6 h-6 ${isGroup ? 'bg-purple-600' : 'bg-primary'} flex items-center justify-center`}
                      >
                    ...
                    <div className="text-start">
                      <div className="flex items-center justify-between gap-1 mb-1">
                        <div
                          className={`font-bold text-sm ${selectedPackage === pkg.id ? (isGroup ? "text-purple-700" : "text-primary") : "text-slate-800"}`}
                        >
                        ...
                        {isGroup ? (
                          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 font-extrabold border border-purple-200">
                            <Users className="w-3 h-3" />
                            {language === 'ar' ? `جماعية (${pkg.studentsNum || 2})` : `Group (${pkg.studentsNum || 2})`}
                          </span>
                        ) : (
            ...
            {selectedPlanObj?.planType === 'group' && (
              <div className="mt-4 p-4 rounded-2xl bg-gradient-to-br from-purple-50 via-indigo-50/60 to-purple-50 border border-purple-200 text-start space-y-3.5 shadow-sm animate-fadeIn">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] bg-purple-200 text-purple-800 px-2 py-0.5 rounded-full font-bold">
                        {selectedPlanObj.studentsNum || 2} {language === 'ar' ? 'طلاب' : 'students'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-1">
                  <label className="block text-xs font-bold text-slate-700">
                    {language === 'ar'
                      ? `دعوة زملائك للمجموعة (حتى ${Math.max(1, (Number(selectedPlanObj.studentsNum || 2) - 1))} أعضاء)`
                      : `Invite Members (up to ${Math.max(1, (Number(selectedPlanObj.studentsNum || 2) - 1))} members)`}
                  </label>
                  ...
                  {groupMembers.length < Math.max(1, (Number(selectedPlanObj.studentsNum || 2) - 1)) && (
                    <button
                      type="button"
